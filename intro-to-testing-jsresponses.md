...
1. Once you're setup and comfortable, go to `code.js` and change the name of the `helloWorld` function to `hello`. Then refresh `report.html` in your browser.
    - What do you notice about the test results?             all went red, all fails 
    - What are some ways you think we could get the tests to turn green again? maybe if we set the test to search for if the function has the word hello
    - Set the function name in `code.js` back to `helloWorld` and re-run the tests.
all went red, all fails.. maybe if we changed the tests to search for if the function even has the word hello?
   considering i should be refracting code not tests... id say changing the name back.
...
only the one failure of course, that the string isnt exact. all others are green.
...
all other tests fail now, this is because the value isnt being returned to a location these tests are expecting.
   console.log returns the value given, but it is given to the console.
   'return' returns the value back to where the call for the function was made. 
...
all fails on error
2.Now, go to `code.js` and replace the function statement for `helloWorld` with a function experession.
      -Do all the tests stay green or not? 
       -Why or why not? 
      - Double check your syntax. These are interchangeable because
        functions are *first class* citizens in the JS language.
      i experienced no change. 
```js
...
- ✅If we have `return "Hello, " + input + "!";`, does this work for all names?
  - ✅Does this bring up any other issues with other inputs?      // numbers will have an issue, anything not a string really..
...
- ✅Recommend checking if the input variable's value is `undefined`.
